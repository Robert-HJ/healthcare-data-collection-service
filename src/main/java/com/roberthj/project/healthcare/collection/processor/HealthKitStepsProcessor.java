...
public class HealthKitStepsProcessor implements HealthDataProcessor {
    ...
    private List<HealthStepDataUpsertRow> createRows(HealthDataCollectionRequestEntity request) {
        ...
        return normalizedData.values().stream()
            .map(data -> new HealthStepDataUpsertRow(member.getId(), requestId, HealthDataSource.HEALTH_KIT,
    ...
    private void aggregate(MemberEntity member, List<HealthStepDataUpsertRow> rows) {
        aggregationService.recalculate(member, HealthDataSource.HEALTH_KIT, rows.stream().map(HealthStepDataUpsertRow::startedAt).toList());
    }
